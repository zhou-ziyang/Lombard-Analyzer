...
基于 `src/weekly/WeeklyAnalysisGenerate.bas` 通读整理（14,039 行 / 234 个过程；模块头部的版本注释停在
v82，之后的改动只在 git 记录里）。
...
颜色分得开。类别节点的颜色不再是饼图的分类色，而是这个类别本月的净变动（进减出）对上月持仓的
比例：涨绿、跌红、没动灰，幅度越小越淡，±20% 起满色（`FLOW_FULL_COLOR_CHANGE`），有变动至少
35% 的浓度（`FlowNetColor`、`BlendTowardsWhite`）；标签末尾用同样的颜色写出净变动百分比，框底
...
的金额红色带减号、进来的绿色带加号；四个侧节点各三行：标题、NDG 数、金额），全部 Group 成一个名为
