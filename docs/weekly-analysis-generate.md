...
基于 `src/weekly/WeeklyAnalysisGenerate.bas` 通读整理（12,157 行 / 197 个过程；模块头部的版本注释停在
v82，之后的改动只在 git 记录里）。
...
表上。每个衡量变化的区块都把周环比放在月环比旁边；Overview 和 Breakdown 从新到旧排，当前
日期在最上面，Breakdown 里每个变化行紧贴它的比较基准那一行。
