...
基于 `src/weekly/WeeklyAnalysisGenerate.bas` 通读整理（13,122 行 / 214 个过程；模块头部的版本注释停在
v82，之后的改动只在 git 记录里）。
...
- **Bond Issuers**：按 ticker 一行。各字段独立累积，所以同一 ticker 下的另一个 ISIN 可以补上
  ...
  给了理由：Sophis 之后可能给出更正过的名字。被覆盖掉的旧名不写进任何表，只在这一次运行里
  记着（`IssuerRenames`，按 ticker），交给下面的改名识别和 New Geo-Sec Lookup。
...
`PreviousNames`（这次运行里 Bond Issuers 被 Sophis 的新名覆盖掉的旧名）对 Companies 的名字和
变体；以及它的 ISIN 候选对 Companies 的 Reference ISIN（只算 Issued / Underlying security，
基金的 ISIN 认的是基金不是母公司）。搭上桥、且新名不只是那行 Name 的另一种写法
（`NamesLookAlike`），就是改名。
...
Companies 里从来没有过的名字、但这次运行里 Bond Issuers 刚从旧名改过来的（比如 Deutsche Post AG
改成 DHL AG，而 Companies 里两个名字都没有）：按普通新公司列出——F / H 是 Bloomberg 公式，没有
老行可复制——只是 `Renamed From` 填上旧名，贴进 Companies 时改名记录一起带过去。旧名不进
Name Variants：旧名是旧公司的。改名记录代码只写在 New Geo-Sec Lookup 上；进 Companies 是人
检查过之后的事，贴过去或者按按钮。
